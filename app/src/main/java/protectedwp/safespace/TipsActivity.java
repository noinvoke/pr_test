...
public class TipsActivity extends Activity {
    ...
    private String getTextTip2() {
        return isRussian 
            ? "2. Не используйте биометрическую разблокировку. Только пароль. Никаких отпечатков и лица. Их могут использовать для разблокировки против вашей воли. Пароль тоже могут если окажут давление, но это сделать сложнее. Чтобы защититься от подобного настройте \"Duress Password\", это функция которая поможет вам стереть данные введя фейковый пароль на том же экране блокировки. Она есть в операционной системе GrapheneOS, а также в приложениях DuressKeyboard и Duress (последний (Duress) гарантий сброса не дает, ведь работает исключительно через спецвозможности, которые могут не сработать, тогда как DuressKeyboard использует их только как вспомогательный инструмент для некоторых дополнительных опций)."
            : "2. Do not use biometric unlock. Only a password. No fingerprints or face. They can be used to unlock the device against your will. A password too in case of pressure on you, but this is harder. To protect yourself aganist such situations, set up a \"Duress Password\". This is a feature that will help you wipe data by entering a fake password on the same lock screen. It is available in the GrapheneOS operating system, and also in the DuressKeyboard and Duress apps (the last one (Duress) does not give any wipe guarantees, because it works only via accessibility services, which might not work, whereas the DuressKeyboard uses them only as an auxiliary tool for certain additional options).";
	}
